    ...
    if (preg_match($username_regex, $username, $user_matches, PREG_OFFSET_CAPTURE)) {
        ...
        if (preg_match($password_regex, $password, $pass_matches, PREG_OFFSET_CAPTURE)) {
            ...
            if (preg_match($email_regex, $email, $email_matches, PREG_OFFSET_CAPTURE)){
                ...
                $user_request = $pdo->prepare("SELECT * FROM users WHERE username = ?");
                $user_request->execute([$username]);
                $check_user = $user_request->fetch();

                if ($check_user['username'] === $username) {
                    echo '<script language="javascript">';
                    echo 'alert("Данное имя пользователя уже занято")';
                    echo '</script>';
                } else {
        ...
        echo '<script language="javascript">';
        echo 'alert("Ваше имя пользователя содержит неподдерживаемые символы")';
        echo '</script>';
    }
}
?>
    ...
    <div class="background">
        <div class="register_container">
            ...
            <form action="register.php" method="POST" class="auth_form">
                ...
                <input class="auth_input" placeholder="Имя пользователя" type="username" name="username" required='true' minlength="8" maxlength="20">
                <h2 class="auth_text">Пароль</h2>
                <input class="auth_input" placeholder="Пароль" type="password" name="password" required='true' minlength="8" maxlength="50">
